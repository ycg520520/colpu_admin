 ...
 * @LastEditTime: 2026-01-15 15:48:51
 *
...
export default function FriendList() {
  ...
  const fetchFriendList = async (params: any) => {
    let data = [];
    let total = 0;
    try {
  ...
  const handdleDel = ({ id, type_code }: any) => {
    Modal.confirm({
      ...
      okType: "danger",
      onOk() {
        apiFriend({ id }, "delete").then(async () => {
  ...
  const onFinish = async (values: any) => {
    ...
    await apiFriend(values, isEdit ? "put" : "post");
    onCancel();
    actionRef.current?.reload!();
  };
  // 弹窗props配置
        ...
        <div className={tableStyles}>
          <ProTable
            ...
            request={({ current: page, ...params }) => {
              return fetchFriendList({ page, ...params, ...searchValues });
            }}
            rowKey="id"
            search={false}
            toolbar={
